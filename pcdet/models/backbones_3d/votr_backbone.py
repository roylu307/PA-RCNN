...
class SubMAttention3d(Attention3d):
    ...
    def forward(self, sp_tensor):
        ...
        attend_features = self.drop_out(attend_features)
        voxel_features1 = voxel_features.clone() + attend_features.squeeze(0)
        voxel_features1 = self.norm1(voxel_features1)
        act_features = self.linear2(self.dropout1(self.activation(self.linear1(voxel_features1))))
        voxel_features2 = voxel_features1.clone() + self.dropout2(act_features)
        voxel_features2 = self.norm2(voxel_features2)
        voxel_features2 = self.output_layer(voxel_features2)
        sp_tensor.features = voxel_features2
        return sp_tensor
