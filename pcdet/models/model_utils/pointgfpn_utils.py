...
class Fnode(nn.Module):
    ...
    def forward(self, x, cur_xyz, cur_xyz_batch_cnt, global_roi_grid_points_list, batch_size) -> torch.Tensor:
...
class PointGiraffeLayer(nn.Module):
    def __init__(self, feature_info, fpn_config, neck_cfg, inner_fpn_channels, outer_fpn_channels, num_levels=5):
    ...
    def forward(self, x: List[torch.Tensor], global_roi_grid_points_list: List[torch.Tensor], batch_size):
